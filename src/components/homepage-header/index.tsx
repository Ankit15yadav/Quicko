...
import SquircleCard from "../squircles/card";
      ...
      >
        <View>
          ...
          <View style={styles.locationButton}>
            <Text style={[styles.color, styles.name_of_location]}>HOME -</Text>

            <View style={styles.current_location}>
              <Text style={[styles.color]}>
                {" "}
                {selectedAddress?.[0]?.name ?? location?.coords.longitude}
              </Text>
      ...
      <View style={styles.rightSection}>
        {RIGHT_SECTION_DATA.map((el, idx) => (
          <SquircleCard
            ...
            key={`${el} + ${idx}`}
          >
            <PressableHaptic onPress={() => router.push(el.href)}>
              <el.Icon />
            </PressableHaptic>
          </SquircleCard>
        ))}
